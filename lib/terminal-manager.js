...
`
  ...
  async processTerminalOutput(terminalId, studentId, data) {
    // Process terminal output for AI analysis
    ...
    // Check for prompt return (but ignore startup prompts)
    const hasPrompt = data.includes('$') || data.includes('#') || data.includes('>');
    const isStartupPrompt = data.includes('student@') && (
      this.terminalBuffers.get(terminalId)?.output?.includes('Welcome to Ubuntu') ||
      this.terminalBuffers.get(terminalId)?.output?.includes('Documentation:') ||
      this.terminalBuffers.get(terminalId)?.output?.includes('sudo <command>')
    );
    
    // Check for AI analysis triggers
    
    // ONLY trigger analysis for real errors or real command prompts (not startup)
    if ((hasPrompt && !isStartupPrompt) || hasError) {
      ...
      setTimeout(async () => {
        try {
          ...
          await this.analyzeRecentActivity(terminalId, studentId);
        } catch (error) {
          console.error(`❌ Error in delayed AI analysis:`, error);
        }
      }, 500);
    }
  }
  ...
  async analyzeRecentActivity(terminalId, studentId) {
    try {
      ...
      // Don't analyze too frequently (reduced from 500ms to 200ms)
      // But allow immediate analysis if there's an error or it's the first command
      const timeSinceLastUpdate = Date.now() - buffer.lastUpdate;
      const hasError = buffer.output.toLowerCase().includes('error') || 
                      buffer.output.toLowerCase().includes('command not found') ||
                      buffer.output.toLowerCase().includes('invalid');
      const isFirstCommand = !this.lastCommands.has(terminalId);
      
      if (timeSinceLastUpdate < 200 && !hasError && !isFirstCommand) {
        console.log(`🔍 Skipping analysis due to timing (${timeSinceLastUpdate}ms < 200ms)`);
        return;
      }
      ...
      if (lastCommand && lastCommand.command) {
        ...
        // Method 1: Look for lines with $ prompt
        for (const line of lines.reverse()) {
          if (line.includes('$') && line.length > 10) {
            const parts = line.split('$');
            if (parts.length > 1) {
              const potentialCommand = parts[1].trim();
              if (potentialCommand && !potentialCommand.includes('student@')) {
        ...
        // Method 2: If still no command, look for common command patterns
        if (!command) {
          const allText = output.toLowerCase();
          const commonCommands = ['ls', 'pwd', 'whoami', 'cd', 'cat', 'mkdir', 'touch', 'rm', 'cp', 'mv'];
          for (const cmd of commonCommands) {
            if (allText.includes(cmd) && !allText.includes(`command not found`)) {
              // More sophisticated check - look for the command after a prompt
              const cmdRegex = new RegExp(`\\$\\s*${cmd}(?:\\s|$)`, 'i');
              if (cmdRegex.test(output)) {
                command = cmd;
                break;
              }
            }
      ...
      // Only analyze real commands, not startup noise or password inputs
      const isStartupNoise = command.includes('Warning: Permanently added') || 
                             command.includes("student@localhost's password") ||
                             command.includes('Welcome to Ubuntu') ||
                             command.includes('Documentation:') ||
                             command.includes('sudo <command>') ||
                             command === 'unknown command' ||
                             command.length <= 1;
      
      const isPasswordInput = command === process.env.STUDENT_PASSWORD || 
                              command === 'haxwarp123';
      
      const shouldAnalyze = command && 
                           !isStartupNoise && 
                           !isPasswordInput;
      
      console.log(`🔍 Analysis decision - Command: "${command}", shouldAnalyze: ${shouldAnalyze}`);
      
      if (shouldAnalyze) {
        console.log(`🤖 Starting AI analysis for command: "${command}"`);
