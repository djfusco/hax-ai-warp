...
`
  ...
  async analyzeRecentActivity(terminalId, studentId) {
    try {
      ...
      // Only analyze if we have a real command or a meaningful error
      // Skip startup messages, SSH warnings, and other noise
      const shouldAnalyze = (command && command !== 'unknown command' && command.length > 2) || 
                           (errorOutput && !output.includes('Warning: Permanently added') && 
                            !output.includes("student@localhost's password"));
