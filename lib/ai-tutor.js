...
No motivational text. Be direct and practical.`;
  ...
  buildWarpLikePrompt(context) {
    if (context.error) {
      // Error case - focus on fixing the problem
      return `Command: ${context.currentCommand}
Error: ${context.error}

Provide a brief helpful fix or explanation (under 25 words):`;
    } else {
      // Success case - provide educational guidance
      return `Command: ${context.currentCommand}
Output: ${context.output ? context.output.substring(0, 200) : 'Command executed successfully'}

Provide a brief educational tip, explanation, or next suggested command (under 25 words):`;
    }
  }
