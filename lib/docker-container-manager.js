...
class DockerContainerManager {
  ...
  async ensureImageExists() {
    try {
      ...
      console.log(`✅ Docker image ${this.imageName} already exists`);
      return true;
    } catch (error) {
      console.log(`❌ Docker image ${this.imageName} not found locally`);
      console.log(`📋 Please run the following commands first:`);
      console.log(`   1. docker login`);
      console.log(`   2. docker pull ${this.imageName}`);
      console.log(`   3. Then restart hax-ai-warp`);
      throw new Error(`Docker image ${this.imageName} not found. Please run: docker pull ${this.imageName}`);
    }
  }
...
`;
    ...
    try {
      ...
      await execAsync(`docker build --build-arg STUDENT_PASSWORD=${studentPassword} -t hax-ai-cyber-lab:latest /tmp -f /tmp/hax-ai-dockerfile`);
